package org.ruben.web_personal.ui.sections.web
        ...
        Box(
            modifier = Modifier
                ...
                .drawBehind {
                    drawLine(
                        brush = Brush.horizontalGradient(
                            0.0f to WebPrimary.copy(alpha = 0.8f),
                            0.8f to WebPrimary.copy(alpha = 0.9f),
                            1.0f to Color.Transparent,
                        ),
            ...
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                SectionTitle(title = "Desarrollo Web", color = WebPrimary)
                Text(
                    text = "Explorando la frontera del desarrollo web mediante Kotlin/Wasm. " +
                            "Creación de aplicaciones web interactivas y de alta fidelidad visual, " +
                            "aprovechando la potencia de WebAssembly para garantizar un rendimiento óptimo y una consistencia total con plataformas móviles.",
        ...
        Column(
            ...
            if (state.projects.isNotEmpty()) {
                state.featuredProject?.let { featured ->
                    ProjectCard(project = featured, accentColor = WebPrimary)
                }
                val remaining = state.projects.filter { !it.featured }
                if (remaining.isNotEmpty()) {
                    WebProjectGrid(projects = remaining)
                }
            }
        }
...
@Composable
private fun WebProjectGrid(projects: List<Project>) {
